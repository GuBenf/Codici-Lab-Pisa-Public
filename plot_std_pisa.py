...
def main(input_files, overwrite=False, log_tot=False, output_file=None, ):
    ...
    for input_file in tqdm(input_files, disable=len(input_files)<2):
        ...
        with tb.open_file(input_file) as f:
            ...
            cf = int(cfg[0]['configuration_in.scan.scan_config.start_column'])
            cl = int(cfg[0]['configuration_in.scan.scan_config.stop_column'])
            rf = int(cfg[0]['configuration_in.scan.scan_config.start_row'])
            rl = int(cfg[0]['configuration_in.scan.scan_config.stop_row'])
            ...
            thr_map = f.root.ThresholdMap[cf:cl,rf:rl]
    ...
    with PdfPages(output_file) as pdf:
        ...
        for input_file, c, t in zip(input_files, cfg, tdac):
            ...
            tt = t + np.random.randint(low=-3, high=3, size=(512, 512))
            m_blocks = get_block_matrix(tt,dx = ddx,dy=ddy)
            x_ti = np.arange(0+ddx/2, tt.shape[1]+ddx/2, ddx)  # X axis values
            y_ti = np.arange(0+ddy/2, tt.shape[0]+ddy/2, ddy)   # Y axis values
            ...
            tt = t + np.random.randint(low=-3, high=3, size=(512, 512))
            del nzc, nzr
            t_dac_rows = [np.mean(tt[:,i]) for i in range(lr)]
            ...
            tt = t + np.random.randint(low=-3, high=3, size=(512, 512))
            t_blocks = get_block_matrix(tt,dx = ddx,dy=ddy).transpose()
            indx_col = [i for i in range(len(t_blocks))]
            indx = [i for i in range(len(t_blocks[0]))]
            for j,p_col in enumerate(t_blocks):
...
if __name__ == "__main__":
    ...
    if args.input_file:  # If anything was given on the command line
        ...
        files.extend(glob.glob("*_interpreted.h5"))
    files.sort()
